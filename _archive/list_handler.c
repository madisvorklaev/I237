...
void remove_entry(entry_t **list_head_ptr, entryid_t *rm_w_id)
{
    ...
    while (current != NULL) {
        if (current->entry_number == *rm_w_id) {
            if (prev == NULL) {
...
/*    kg_t t_gargo = 0;*/
    ...
    while (current != NULL) {
        ...
        uart0_putc(current->entry_number);
        uart0_puts_p(PSTR("\r\n"));
/*        printf("Wagon No. %d ID: %d\n", ++w_in_train, current->w_u_id);*/
        total_users += current->user->uid;
        current = current->next;
    }
...
/*    scanf("%s %c", s_buf, n_buf);*/
/*    user_ptr = add_user(s_buf, n_buf);*/
/*    print_user(user_ptr);*/
...
/*    for (unsigned int i = 0; i < *n_buf; i++) {*/
...
/*        remove_entry(&list_head_ptr, &l_buf);*/
/*        print_list(list_head_ptr);*/
/*    }*/
